...
const benefitEntryCardShine = document.querySelector(
...
function loadSubmittedRequest() {
    try {
        return JSON.parse(
            localStorage.getItem(storageKey("submitted-request")) ?? "null",
        );
    } catch {
        return null;
    }
...
function triggerPendingDownload() {
    if (
        !submittedRequest?.autoDownloadPending ||
        !submittedRequest.downloadUrl
    ) {
        return;
    }
...
function setScreen(screenName) {
    ...
    screens.forEach((screen) => {
        screen.classList.toggle(
            "is-active",
            screen.dataset.screen === activeScreen,
        );
    });
...
function setStep(stepNumber) {
    ...
    stepItems.forEach((item) => {
        item.classList.toggle(
            "is-active",
            item.dataset.stepTarget === activeStep,
        );
    });
...
function renderBenefitDetail() {
    ...
    benefitItems.forEach((item) => {
        item.classList.toggle(
            ...
            item.dataset.benefitId === benefit.id,
        );
    });
...
function renderStatusScreen() {
    if (!submittedRequest) {
        ...
        statusBindings.review.textContent =
            "После отправки заявка появится здесь";
        statusBindings.approval.textContent =
            "Статус будет обновлен после проверки";
    ...
    statusBindings.submittedAt.textContent = formatSubmittedAt(
    ...
    statusBindings.payout.textContent =
        "PDF сформирован и доступен для повторной отправки";
    statusBindings.lastRequestTitle.textContent = submittedRequest.benefitTitle;
    statusBindings.lastRequestDate.textContent = formatSubmittedAt(
        submittedRequest.submittedAt,
    );
...
async function submitApplication(button) {
    ...
    try {
        ...
        submittedRequest = {
            statementNumber:
                result.statement_number ?? payload.statement_number,
        ...
        renderStatusScreen();
        setSubmitStatus(
            "Заявление сформировано и сохранено на сервере.",
            "success",
        );
        setScreen("status");
    } catch (error) {
        console.error(error);
        setSubmitStatus(
            "Не удалось сформировать заявление. Проверьте, что benefits-api запущен.",
            "error",
        );
    } finally {
